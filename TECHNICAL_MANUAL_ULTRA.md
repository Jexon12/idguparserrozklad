...
- `index.html` — єдиний адаптивний UI, головний центр керування.
- `index2.html` — compatibility redirect для старих посилань.
...
- Desktop/mobile використовують одну адресу `index.html` і CSS media queries.
- `index2.html` перенаправляє на `index.html`, зберігаючи корисні query-параметри та hash.
...
## 4.3 `js/search.js`
- звертається до серверного індексу `/api/search`;
- скасовує застарілі запити під час швидкого введення;
- за помилки сервера переходить на локальний префіксний індекс.
