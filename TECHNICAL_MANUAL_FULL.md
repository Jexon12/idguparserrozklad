...
## 2.1 `index.html` (адаптивний інтерфейс)
...
- `Звіт` — синхронне формування і завантаження XLSX через backend; локальний
  job-режим збережено лише для сумісності.
- `Скан` (аудиторії) — запуск скану зайнятості.
- `Share` — формує URL зі станом.
- `Calendar` — експорт у календар.
- `Очистити` — очищає локальні дані.

## 2.2 `index2.html` (сумісність)
Мінімальна сторінка-редирект на `index.html` для старих закладок. Власної копії Vue-шаблону більше немає.
...
## 2.6 Підготовка даних у `session-constructor.html`
Перший крок конструктора замінює колишню окрему сторінку Session Prep:
- автоматично збирає з розкладу таблицю `Предмет | Група | Викладачі`,
- підтримує фільтри груп/курсів і діапазон дат,
- передає результат одразу до редагування, перевірки та публікації.

`session-prep.html` залишено як compatibility-route з переходом у конструктор.

## 2.7 Smart Day у `index.html`
Вбудований аналітичний модуль:
- рейтинг зручності,
- теплокарта,
- what-if симулятор,
- live board для змін.

Старий `smart.html` є compatibility-route. Live board диспетчера обслуговується
сторінкою `course-live.html`.
...
## 3.3 `js/search.js`
Індексація/пошук:
- основний пошук виконується через `/api/search` у серверному префіксному індексі;
- запити при швидкому введенні скасовуються;
- локальна побудова каталогу залишається fallback при недоступності сервера.
...
## 3.5 `js/report.js`
Робота зі звітом:
- перевірка параметрів і допустимого діапазону місяців,
- прямий запит сформованого XLSX через `/api/report/download`,
- завантаження отриманого Blob без фонового polling у production.
...
2. Відкрити `index.html` у широкому та вузькому viewport, потім `session.html`, `session-admin.html`, `session-constructor.html`, `course-live.html`.
...
- Швидка PDF-версія: `docs/print/USER_GUIDE_PDF.html`
