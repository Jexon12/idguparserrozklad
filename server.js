...
const server = http.createServer(async (req, res) => {
    ...
    // Cache-Control: HTML always revalidates, JS/CSS moderate cache, media longer cache.
    if (file.mime === 'text/html') {
        headers['Cache-Control'] = 'no-cache, must-revalidate';
    } else if (file.mime === 'text/css' || file.mime === 'text/javascript') {
        headers['Cache-Control'] = 'public, max-age=3600, must-revalidate';
    } else {
        headers['Cache-Control'] = 'public, max-age=86400';
    }

    // Prefer brotli, then gzip, then plain.
    const acceptsGzip = (req.headers['accept-encoding'] || '').includes('gzip');
    const acceptsBrotli = (req.headers['accept-encoding'] || '').includes('br');
    if (acceptsBrotli && file.brotli) {
        headers['Content-Encoding'] = 'br';
        res.writeHead(200, headers);
        res.end(file.brotli);
    } else if (acceptsGzip && file.gzipped) {
