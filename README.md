...
`vercel.json` явно задає `framework: null`, команду `npm run build` та каталог
результату `public`. Збірка створює цей каталог із публічних файлів; `api/`
залишається в корені для Vercel Functions. Не встановлюйте Output Directory
на корінь репозиторію: там є серверні та локальні файли. `public/` генерується
автоматично та не комітиться. Для перевірки перед деплоєм запустіть `npm run build`.
